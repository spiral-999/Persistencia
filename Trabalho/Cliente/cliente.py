...
def testar_cliente():
    try:
        ...
        filme_atualizado = {"nome": "O Exorcista (Director's Cut)", "categoria": "Terror Clássico", "nota_imdb": 8.2}
        r_put = httpx.put(f"{BASE_URL}/filmes/{novo_id_1}", json=filme_atualizado)
        print(f"Status: {r_put.status_code}")
        print(r_put.json())

        print_separador(f"GET /filmes/{novo_id_1} (Verificar atualização)")
        ...
        print_separador(f"DELETE /filmes/{novo_id_1} (Remover O Exorcista)")
        ...
        dados_maior_lista = r_maior.json()
        primeiro_filme_maior = dados_maior_lista[0] 
        print(f"Filme de maior nota: {primeiro_filme_maior['nome']} (Nota: {primeiro_filme_maior['nota_imdb']})")
        ...
        dados_menor_lista = r_menor.json()
        primeiro_filme_menor = dados_menor_lista[0]
        print(f"Filme de menor nota: {primeiro_filme_menor['nome']} (Nota: {primeiro_filme_menor['nota_imdb']})")
        ...
        lista_acima = r_acima.json()
        print(f"Total de filmes acima da média: {len(lista_acima)}")
        print("Filmes:")
        # descomentar para mostra a lista com todos acima da media
        #for filme in lista_acima:
        #    print(f"  - {filme['nome']} (Nota: {filme['nota_imdb']})")
        ...
        lista_abaixo = r_abaixo.json()
        print(f"Total de filmes abaixo da média: {len(lista_abaixo)}")
        print("Filmes:")
        # descomentar para mostra a lista com todos abaixo da media
        #for filme in lista_abaixo:
        #    print(f"  - {filme['nome']} (Nota: {filme['nota_imdb']})")
